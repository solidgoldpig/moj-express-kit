...
router.get('/admin/elements/:blockType', (req, res) => {
  let keys = elementsDal.getKeys(req.params.blockType).sort()
...
router.get('/admin/elements', (req, res) => {
  let blockTypes = elementsDal.getBlockTypes().sort()
  res.render('admin/element-types/element-types', { blockTypes })
})
router.get('/admin/element/:element', (req, res) => {
  ...
  if (allProps.length === 1 && typeof element[allProps[0]] === 'string') {
...
router.get('/api/element/:blockType/:value', (req, res) => {
  res.json(elementsDal.getAll(req.params.blockType, req.params.value) || [])
})
        ...
        .then(routeOutcome => {
          ...
          if (!errors && req.method === 'POST' && routeInstanceFinal.redirect && req.originalUrl !== routeInstanceFinal.redirect && req.body.updateForm !== 'yes') {
            ...
            nunjucksEnv.addFilter('makeBlockEditable', function(str, name, type) {
              if (isEdit) {
                str = str.replace(/(\s*<[^>]+)/, '$1 data-element-type="' + type + '"')
              }
              return str
            })
            ...
            res.render(`templates/${template}/${template}`, {
              ...
              if (err) {
                ...
                res.send(rendered)
                // TODO: GET ENV
                if (routeInstanceFinal.static && process.env.ENV) {
