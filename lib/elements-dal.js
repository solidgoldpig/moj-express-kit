...
function processElements () {
  ...
  function extendElement (key) {
    ...
    if (element._isa) {
      let extendedKey = element._isa
      let extendedElement = elements[extendedKey]
      if (extendedElement._isa && !processedElements[extendedKey]) {
...
function getBlockTypes () {
  ...
  let keys = Object.keys(elementsSrc).map(key => {
    return elementsSrc[key]._blockType
  }).filter(type => {
...
function getKeys (type) {
  ...
  if (type) {
    keys = keys.filter(key => {
      return elementsSrc[key]._blockType === type
    })
  }
  return keys // .sort()
}
